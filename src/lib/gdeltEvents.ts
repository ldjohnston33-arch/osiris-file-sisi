...
import { getBuffer } from '@/lib/osiris/http';
...
function httpGetBufferIPv4(url: string, timeoutMs: number): Promise<Buffer> {
  // Shared client: IPv4-pinned, follows GDELT's http->https redirect, honours HTTPS_PROXY.
  return getBuffer(url, { timeoutMs });
}
...
async function latestExportUrl(): Promise<string> {
...
export interface WindowOptions extends FetchOptions {
  /** How many consecutive 15-minute exports to read (newest first). */
  windows?: number;
  /** Bounding box filter [minLng, minLat, maxLng, maxLat]. */
  bbox?: [number, number, number, number];
  /** Keep rows where either actor carries one of these CAMEO country codes,
   *  or the action geo falls in bbox. */
  actorCountries?: string[];
}

/**
 * Reads several consecutive 15-minute exports and keeps rows that fall in a
 * region of interest. One export is ~15 minutes of global coverage; the
 * Osiris File reads a few hours so thin regions still show up.
 */
export async function fetchGdeltWindows(opts: WindowOptions = {}): Promise<GdeltEventsResult & { windowsRead: number }> {
  const { windows = 8, bbox, actorCountries = [], quads = [], minArticles = 1, limit = 1500 } = opts;
  const latest = await latestExportUrl();
  const urls = [latest, ...Array.from({ length: windows - 1 }, (_, i) => previousWindowUrl(latest, i + 1)).filter((u): u is string => !!u)];
  const events: GdeltEvent[] = [];
  let scanned = 0;
  let windowsRead = 0;
  for (let i = 0; i < urls.length; i += 4) {
    const batch = await Promise.allSettled(urls.slice(i, i + 4).map(u => httpGetBufferIPv4(u, 20000).then(unzipSingleEntry)));
    for (const r of batch) {
      if (r.status !== 'fulfilled') continue;
      windowsRead++;
      const rows = r.value.split('\n');
      scanned += rows.length;
      for (const e of parseRows(rows, { quads, minArticles, limit: Infinity })) {
        const inBox = bbox ? e.lng >= bbox[0] && e.lat >= bbox[1] && e.lng <= bbox[2] && e.lat <= bbox[3] : true;
        const actorHit = actorCountries.length > 0 && (actorCountries.includes(e.actor1Country) || actorCountries.includes(e.actor2Country));
        if (inBox || actorHit) events.push(e);
      }
    }
  }
  return { events: events.slice(0, limit), window: urls[0].split('/').pop() || '', scanned, windowsRead };
}
...
export async function fetchGdeltEvents(opts: FetchOptions = {}): Promise<GdeltEventsResult> {
  ...
  const events = parseRows(rows, { quads, minArticles, limit });
...
function parseRows(rows: string[], { quads = [], minArticles = 1, limit = 600 }: FetchOptions): GdeltEvent[] {
  const events: GdeltEvent[] = [];
