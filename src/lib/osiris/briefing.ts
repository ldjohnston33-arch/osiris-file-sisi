...
Write a 3-4 sentence briefing on the last seven days from the supplied events only.
Rules: lead with the most consequential development and why it matters for Egypt. Reference concrete events with dates. Direct, assertive, pragmatic tone; no throat-clearing. No em dashes. No quotation marks and no invented statements: describe what he did or what the record shows, never what he thinks. Do not use negative-contrast constructions ("not X but Y").
Return JSON: {"briefing":"<paragraph>"}`;
...
export function rulesBriefing(events: OsirisEvent[], partners: PartnerRank[], risk: RiskBreakdown[], now = Date.now()): Briefing {
  ...
  if (trips.length) {
    ...
    parts.push(`Over the past week Sisi's travel took him to ${places.join(', ')}, most recently on ${fmtDay(trips[0].date)}.`);
  } else {
    parts.push('Sisi has made no tracked foreign trips in the past week.');
  }
  if (inbound.length) {
    const who = [...new Set(inbound.map(i => i.partner).filter(Boolean) as string[])].slice(0, 4).map(countryName);
    parts.push(`Cairo received ${inbound.length} foreign visit${inbound.length > 1 ? 's' : ''}${who.length ? `, including delegations from ${who.join(', ')}` : ''}.`);
  }
  ...
  if (busiest) {
    ...
    parts.push(`${label[busiest[0]] ?? busiest[0]} generated the most regional activity in the tracker (${busiest[1]} item${busiest[1] === 1 ? '' : 's'}).`);
  }
  if (partners[0]) parts.push(`Over 90 days, ${partners[0].name} remains the most engaged partner by visit count.`);
  if (eg) parts.push(`L4's composite risk reading for Egypt stands at ${eg.score}/100 (${eg.level.toLowerCase()}).`);
  return { text: parts.join(' '), method: 'rules', generatedAt: new Date(now).toISOString() };
}
